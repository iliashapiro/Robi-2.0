<div ng-controller="${pageNameClean}Ctrl">
  ...
  <#list view.getTitles() as item>
    ...
    <#if (item.getSubset()?length > 0)>
      <div>
        <tm1-ui-subnm tm1-instance="${instance}" tm1-dimension="${item.getDimension()}" tm1-subset="${item.getSubset()}" tm1-default-element="${item.getSelected()!}" ng-model="page.titles['${item.getDimension()}']"></tm1-ui-subnm>
      </div>
    <#else>
      <div>
        <select class="form-control" ng-model="page.titles['${item.getDimension()}']">
  ...
  <h3>Template</h3> 
    ...
    <#if (item.getSubset()?length > 0)>
      <tm1-ui-element-list tm1-calculate-top-level="true" tm1-instance="${instance}" tm1-dimension="${item.getDimension()}" tm1-subset="${item.getSubset()}" ng-model="page['${item.getDimension()}']"></tm1-ui-element-list>
    </#if>
    </#list>
  
    <#list view.getRows() as item>
    <#if (item.getSubset()?length > 0)>
      <tm1-ui-element-list tm1-calculate-top-level="true" tm1-instance="${instance}" tm1-dimension="${item.getDimension()}" tm1-subset="${item.getSubset()}" ng-model="page.rows"></tm1-ui-element-list>
      <#break>
    </#if>
    </#list>
    
    
    <tm1-ui-progress tm1-ui-trigger="page.titles">
      ...
      <tm1-ui-progress-ready>
        ...
        <table class="table table-striped" >
          <tr>
            ...
            <#list view.getColumns() as item>
            <#if (item.getSubset()?length > 0)>
              <th ng-repeat="item in page['${item.getDimension()}']" class="text-right">{{item.alias}}</th>
            <#else>
              <#list item.getElements() as element>
          ...
          <tr ng-repeat="row in table.data()" ng-class="{'consol': !row.isLeaf}">
            ...
            <#list view.getColumns() as item>
                <td ng-repeat="column in page['${item.getDimension()}']" class="text-right">
                  <tm1-ui-dbr 
